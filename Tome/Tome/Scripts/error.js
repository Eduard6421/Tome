...
var cardNodes = document.querySelectorAll('.card-container');
var perspecNodes = document.querySelectorAll('.perspec');
var perspec = document.querySelector('.perspec');
var card = document.querySelector('.card');

var counter = stackContainer.children.length;
    ...
    if (e.animationName === 'explode') {
        cardNodes.forEach(function (elem, index) {
            ...
            elem.addEventListener('click', function () {
                var updown = [800, -800]
                var randomY = updown[Math.floor(Math.random() * updown.length)];
                var randomX = Math.floor(Math.random() * 1000) - 1000;
            ...
            var numLines = randomIntFromInterval(5, 10);

            //loop through the lines and add them to the DOM
            for (let index = 0; index < numLines; index++) {
                var lineLength = randomIntFromInterval(25, 97);
